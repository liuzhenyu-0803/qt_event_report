...
void EventReportService::reportEvents(const QList<TrackEvent>& events)
{
    ...
    {
        eventsArray.append(createEventObject(event.eventType, event.eventProperties, event.userProperties));
    }
...
void EventReportService::reportEventsFromJsonFile(const QString& filePath)
{
    ...
    while (!in.atEnd())
    {
        ...
        {
            for (const auto& val : doc.array()) eventsArray.append(val.toObject());
        }
    }
    file.close();
...
void EventReportService::sendEvents(const QJsonArray& eventsArray)
{
    ...
    for (int i = 0; i < totalEvents; i += maxBatchSize)
    {
        ...
        for (int j = 0; j < currentBatchCount; ++j) currentBatch.append(eventsArray.at(i + j));

        QJsonObject root;
        root["api_key"] = m_configService->getAmplitudeApiKey();
        root["events"] = currentBatch;

        QString endpoint = m_configService->getEventTrackEndpoint();
        qInfo() << "EventReportService:sendEvents: Sending batch of" << currentBatchCount << "events to" << endpoint;

        QJsonDocument doc(root);
        QNetworkReply* reply = m_httpService->post(endpoint, doc.toJson());

        connect(reply, &QNetworkReply::finished, this, [this, reply, currentBatch]()
        {
...
void EventReportService::onReplyFinished(QNetworkReply* reply, const QJsonArray& eventsData)
{
...
void EventReportService::saveEventsToFile(const QJsonArray& eventsData)
{
...
QJsonObject EventReportService::createEventObject(const QString& eventType, const QVariantMap& eventProperties, const QVariantMap& userProperties)
{
    QJsonObject eventObj;
    eventObj["user_id"] = m_identityService->getUserID();
    eventObj["device_id"] = m_identityService->getDeviceID();
