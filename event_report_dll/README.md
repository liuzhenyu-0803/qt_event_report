...
**性能优化设计**：
*   **无锁化设计**：由于所有服务均运行在同一个工作线程内，服务内部状态的读写已在线程维度实现串行化。因此，`IdentityService` 和 `ConfigService` 采用了无锁设计，消除了互斥锁（Mutex）带来的上下文切换开销。
*   **集中化配置**：所有注册表路径、API 键名及服务端点均统一在 `event_report_constants.h` 中管理，方便维护。
